...
class fileobj(models.Model):
    ...
    edittype = models.BooleanField(
        default=False, verbose_name="修改方式,True为追加,False为覆盖")
    rpath = models.CharField(max_length=1000, verbose_name="远程主机的路径")
    bcmd = models.CharField(
        max_length=1000, default='', verbose_name="读改前运行的命令")
    acmd = models.CharField(
        max_length=1000, default='', verbose_name="修改后运行的命令")
    ...
    def MakeInput(Random, FileName, RPath, edittype, BeforeCMD, AfterCMD):
        ...
        td += [u'文件路径:', MakeInput('text', 'rpath', RPath, 'width:250px')
               + MakeAjaxButton('a', '#', u'选择', 'load', Random), ]
        td += [u'编辑方式:',
               MakeRaio('edittype', ['覆盖', '追加'], ['write', 'append'], edittype), ]
...
class scripts(models.Model):
    ...
    def GetScriptPath(cls, Script, TmpScript=False):
        ...
        if TmpScript and not os.path.isfile(ScriptFile):
            open('/tmp/TmpScript', 'wb').write(Script.encode('utf-8')
                                               .replace('\r\n', '\n'))
