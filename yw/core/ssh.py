...
Pk_path = os.path.join(
    os.path.dirname(os.path.dirname(os.path.realpath(__file__))), 'rsa_key')
...
def ssh_pexcept(host, port, user, password, sudo, timeout=60, title=True, cmd='id'):
    ...
    child = pexpect.spawn('ssh  -t -o StrictHostKeyChecking=no -p %s %s@%s' %
                          (port, user, host))
    # 获取命令执行结果
    cmd = '%s  echo Rnum:0' % cmd if cmd.endswith(
        r'&') else '%s ; echo Rnum:$?' % cmd
    s.append('[%s@%s] -e- Run Cmd: ' % (user, host) if title else '')
    while True:
                i = child.expect(
                    ['.*assword:', '.*[$#>]', pexpect.EOF, pexpect.TIMEOUT], timeout=timeout)
                if i == 0:
                        ...
                        t = child.before.decode('utf-8', 'ignore') + \
                            child.after.decode(
                                'utf-8', 'ignore') + '\n'
                        # 替换终端特殊字符
                        t = re.sub('\[[\d]+;[\d]+m|\[[\d]{0,}m', '', t)
                        s.append(t)
                        child.sendline('exit')
                        child.sendline('exit')
...
class Host(paramiko.SSHClient):
    ...
    def _Connect(method):  # 执行方法前都先连接到主机
        def Action(self, *args, **kwargs):
            stime = time.time()
            try:
                self.connect(self.host, self.port, self.user,
                             self.password, pkey=self.key, timeout=self.timeout)
                ...
                # print sys.exc_info()
                r = 'Connect [%s@%s] Error!\n%s' % (
                    self.user, self.host, str(sys.exc_info()))
                RetCode = 555
            finally:
                self.close()
                etime = time.time()
    ...
    def _runcmd(self, title=True, cmd='id', redirect=''):  # 执行命令
            ...
            if self.sudo:
                cmd = u'''sudo su - -c "%s"''' % cmd.replace(r'"',
                                                             r'\"').replace(r'$', r'\$')
            stdin, stdout, stderr = self.exec_command(
                cmd.encode('utf-8'), bufsize=-1, timeout=self.timeout * 5)  # 处理时间超时
    ...
    def RunScript(self, title=True, script='', savepath='', daemon=False, scripttype=''):
            ...
            # 后台记录文件
            ScriptLog = os.path.join(savepath, '%s_daemon' % ScriptFile)
            Daemon = ''
            info = ''
            if daemon:
