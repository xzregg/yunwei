...
def TextToHostConfig(Text, sudo, commit=False):
    ...
    for line in Text.split('\n'):
        ...
        if ma:  # 匹配组开头符号
                ...
                continue
        # 不是开头#也不是组开头符号
        elif not re.search('^#', line) and re.search('\w', line):
                    host = line.split()
                    UserIp = '%s@%s' % (host[1], host[3])
                    k = str(i) + ' ' + UserIp
                    Hosts.setdefault(k, {})
                    d = Hosts[k]
                    d['alias'], d['user'], d['passwd'], d[
                        'ip'], d['port'] = host[:5]
                    d['describe'] = ' '.join(host[5:])
                    d['sshtype'] = 1 if d['user'] != 'root' and sudo else 0
                    d['group'] = ',' + ','.join(groups) + ','
                    o = None
                    # and not hosts.objects.filter(ip=d['ip'],user=d['user']):
                    if commit:
...
def HostAttr(request):
    ...
    if hid and ip:
        Webssh = WebSSHFormat % (
            MakeURL(RedirectToWebSsh) + u'?hid=%s' % hid, ip)
    ...
    InterfaceURL = MakeURL(Interface) + \
        (Urlopt if hid else '?SelectHostGroup=%s' % group)
...
def HostsManage(request):
    ...
    table = {'hosts':
             ['checkall', 'alias', 'user', 'ip', 'port', 'group', 'describe']}
    thcss = ['width:50px', 'width:100px', 'width:100px',
             'width:100px', 'width:40px', 'width:200px', '']
    T = Table(table, alias, thcss)  # 第3项为th的style
    HostAttrURL = MakeURL(HostAttr)
    RedirectToWebSshURL = MakeURL(RedirectToWebSsh)
    for v in Hosts:
            checkbox = MakeCheckbox('%s__%s__%s__%s__%s' %
                                    ('hosts', 'ip', v.ip, 'id', v.id), '')
            HostAttrAjax = MakeAjaxButton(
                'a', HostAttrURL + '?hid=%s&ip=%s' % (v.id, v.ip), v.alias, 'load', Random)
            Webssh = WebSSHFormat % (
                RedirectToWebSshURL + u'?hid=%s' % v.id, v.ip)
            td = [checkbox, HostAttrAjax, v.user,
                  Webssh, v.port, v.group, v.describe]
            tdattr = ['', '', '', '', '', '', 'class="text_left"']
            T.row_add(v.id, td, tdattr)
    T.done()
    hidecolume = [
        '<a href="javascript:void(0);" class="hidecolumn" cloumn="2">用户</a>']
...
def Modify(request):
    ...
    if tablename:
        ...
        if row and cloumn and td and value:
            s = eval('''%s.objects.filter(%s=%s).update(%s=value)''' %
                     (tablename, th, td, cloumn))
...
def AddHost(request):
    ...
    if request.method == "POST":
        ...
        if user or password:
            ...
            password = privatekey_decodeing(
                password, privkey) if password else password
        if action == "AddHost":
                if ip and port and user and password and not hosts.objects.filter(ip=ip, user=user):
                    ...
                    if c == 0:
                        ...
                        Newrequest.POST['hid'], Newrequest.POST[
                            'ip'] = (str(o.id), ip)
    ...
    if hid:
        ...
        sudocheckbox = MakeCheckbox(
            'sudo', u'sudo为root', True if o.sshtype == 1 else False)
        button = [
            [u'修改主机', MakeURL(AddHost) + '?action=EditHost&hid=%s' % hid]]
    else:
...
def DelHost(request):
    ...
    if action == "Del":
        ...
        button = [
            [u'删除', MakeURL(DelHost) + u'?action=Del', 'load', u'所属的文件对象也会删除！', 'center', 3]]
...
def AddHostGroup(request):
    ...
    Random = int(time.time() * 100)
    if request.method == "POST":
        hostgroupname, hostslist, describe = GetPost(
            request, ['hostgroupname', 'checkbox', 'HostDescribe'])
        if hostgroupname:
            if not hostgroup.objects.filter(groupname=hostgroupname):
                    hostgroup.objects.create(
                        groupname=hostgroupname, groupmembers='', grouodescribe=describe)
                    SettingHostGroup()
...
def DelHostGroup(request):
    ...
    button = [
        [u'确定删除', MakeURL(DelHostGroup) + u'?action=Del', 'load', u'所属的文件对象也会删除！', 'center', 3]]
    return render_to_response('yw_Del.html', locals())
...
def HostGroupManage(request):
    ...
    table = {'hostgroup':
             ['checkall', 'groupname', 'groupmembers', 'grouodescribe']}
    ...
    for o in hostgroup.objects.all():
            checkbox = u'''<input type='checkbox' class='checkbox' name='checkbox' value='%s__%s__%s__%s__%s'/>''' % (
                'hostgroup', 'groupname', o.groupname, 'id', o.id)
            ajax1 = MakeAjaxButton(
                'a', MakeURL(HostGroupAttr) + '?gid=%s&group=%s' %
                (o.id, o.groupname), o.groupname, 'load', Random)
            groupmembers = len([x for x in o.groupmembers.split(',') if x])
            ajax2 = MakeAjaxButton(
                'a', MakeURL(ShowGroupHosts) + '?gid=%s&group=%s' %
                (o.id, o.groupname), u'查看成员[%s]' % groupmembers, 'open', Random)
