...
def ShowOneKey(request):
	...
	for o in onekeys.objects.all():
		...
		OneKeyButton={'ajaxurl':MakeURL(RunOneKey)+u'?Oid=%s&onekeyname=%s'%(o.id,o.onekeyname),
...
def RunOneKey(request):
	...
	if action==u'RunOneKey':
		if o.checktype==1:
			...
			if not verify==checkcode:return HttpResponse(u'验证码不正确！')
		Ret=[]
	...
	if o.checktype==1:
		RunOneKeyCheck=True
	button=[[u'运行[%s]'%o.onekeyname,MakeURL(RunOneKey)+u'?action=RunOneKey&Oid=%s&onekeyname=%s'%(o.id,o.onekeyname),'load',u'运行一键','center'],]
...
def OneKeyManage(request):
	...
	table={'onekeys':['checkall','onekeyname','onekeygroup','onekeydescribe','creater','createtime']}
	alias={u'主机组':[checkall,u'一键名',u'目标组',u'描述',u'创建者',u'创建时间']}
	thcss=['','','','width:400px','']
	T=Table(table,alias,thcss)
	for o in onekeys.objects.all():
			checkbox=u'''<input type='checkbox' class='checkbox' name='checkbox' value='%s__%s__%s__%s__%s'/>'''%('onekeys','onekeyname',o.onekeyname,'id',o.id)
			ajax=MakeAjaxButton('a','/yw/onekey/EditOneKey/?Oid=%s&onekeyname=%s'%(o.id,o.onekeyname),u'修改','open',Random)
			ajax1=MakeAjaxButton('a','/yw/onekey/RunOneKey/?Oid=%s&onekeyname=%s'%(o.id,o.onekeyname),u'运行','load',Random)
			td=[checkbox+ajax+ajax1,o.onekeyname,o.onekeygroup,o.onekeydescribe,o.creater,o.createtime]
			tdattr=['','','','','']
...
def EditOneKey(request):
	...
	if request.method=='POST':
		onekeyname,onekeyurl,onekeygroup,checktype,onekeydescribe=GetPost(request,
		...
		if onekeyname and onekeyurl and Oid:
			...
			onekeys.objects.filter(id=Oid).update(onekeyname=onekeyname,checktype=checktype,onekeyurl=onekeyurl,onekeygroup=onekeygroup,creater=Creater,onekeydescribe=onekeydescribe,createtime=Createtime)
...
def MakeOneKeyInput():
	input=[]
	input.append(MakeSelect('cmdtimeout',['10','30','60'],['10','30','60'],0,u'连接超时'))
	input.append(MakeSelect('thread',['5','50','200'],['5','50','200'],0,u'线程'))
...
def AddOneKey(request):
	...
	if request.method=='POST':
		...
		if onekeyname and onekeyurl :
			...
			onekeys.objects.create(onekeyname=onekeyname,checktype=checktype,onekeyurl=onekeyurl,onekeygroup=onekeygroup,creater=Creater,onekeydescribe=onekeydescribe,createtime=Createtime)
