...
def Iptables(request):
	...
	if create:#新创建
		if hid:
			...
			port=22
			edittype=True
			bcmd=u"sed -i '/^#<yw_%s>/,/^#<\/yw_%s>/d' %s"%(mark,mark,rpath) 
			acmd=u"iptables -nvL && sed -n '/^#<yw_%s>/,/^#<\/yw_%s>/p' %s"%(mark,mark,rpath) 
...
#iptables -A firewall -p tcp --dport %s -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
		'''%port
		o.save()
	NewRequest=Myrequest(request);rP=NewRequest.POST
	rP['fid']=str(o.id)
	rP['rpath']=o.rpath
...
def Cron(request):
		...
		if create:#新创建
			if hid:
				edittype=False;Gbcmd='';Gacmd=''
			else:#主机组默认为追加
				edittype=True
				Gbcmd=u"&& sed -i '/^#<yw_%s>/,/^#<\/yw_%s>/d' %s"%(mark,mark,rpath) 
				Gacmd=u''
				#Gacmd=u"&& sed -n '/^#<yw_%s>/,/^#<\/yw_%s>/p' %s"%(mark,mark,rpath) 
...
def FileOBJ(request):
	...
	hid,ip,gid,group,HostsId,fid,action,filename,rpath,edittype,FileObjText,AfterCMD,BeforeCMD=GetPost(request,
	...
	if fid :
		...
		if action=="Del":
			...
			o.filename=filename;o.edittype=edittype;o.rpath=rpath;o.text=FileObjText.encode('utf-8');o.bcmd=BeforeCMD;o.acmd=AfterCMD
			o.save()
			...
			if hid:
					...
					HostObj=Host(Ho.ip,Ho.port,Ho.user,Ho.password,Ho.sshtype,10)
					c,FileObjText,t=HostObj.FileOBJ('read',o.rpath,'',o.bcmd)#主机直接读远程的
					etime=time.time()
			...
			button=[
				[u'保存本地',MakeURL(FileOBJ)+Urlopt+'&action=Modify&fid=%s&rpath=%s'%(fid,rpath),'load',u'保存到数据！','center',3],
				[u'同步远程',MakeURL(FileOBJ)+Urlopt+'&action=Sync&fid=%s&rpath=%s'%(fid,rpath),'load',u'同步对端!','center']
				]
...
def AddFileOBJ(request):
	...
	if ((hid and ip) or (gid and group)) and request.method=="POST" and filename and rpath and edittype:
		...
		if edittype=="append":
			edittype=True 
			bcmd="sed -i '/^#<yw_%s>/,/^#<\/yw_%s>/d' %s"%(mark,mark,rpath) 
			acmd="sed -n '/^#<yw_%s>/,/^#<\/yw_%s>/p' %s"%(mark,mark,rpath)
		else:
			edittype,bcmd,acmd=(False,'','')
		if fileobj.objects.filter(unitsid=unitsid,filename=filename):return HttpResponse(u'[%s]已存在!'%filename)
		o=fileobj.objects.create(isgroup=isgroup,unitsid=unitsid,filename=filename,edittype=edittype,rpath=rpath,bcmd=bcmd,acmd=acmd)
...
def ShowFileOBJ(request):
	...
	for o in fileobj.objects.filter(unitsid=unitsid):
		FileObjs.append(MakeAjaxButton('a',MakeURL(FileOBJ)+Urlopt+'&fid=%s&rpath=%s'%(o.id,o.rpath),o.filename,'load',Random,'','center'))
		FileObjs.append(MakeAjaxButton('a',MakeURL(FileOBJ)+Urlopt+'&fid=%s&rpath=%s&action=Del'%(o.id,o.rpath),u'删除','load',Random,u'删除','center',3))
