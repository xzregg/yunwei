...
def DownloadScript(request):
    ...
    Sid, Scriptname, Selects = GetPost(
        request, ['sid', 'scriptname', 'checkbox'], [2])
    if Sid:
        o = scripts.objects.get(id=Sid)
        response = HttpResponse(mimetype='text/script')
        response[
            'Content-Disposition'] = 'attachment; filename=%s' % o.scriptname
...
def ScriptUL(Random):
    ...
    for o in scripts.objects.all():
        if not d.has_key(o.scriptgroup):
            d[o.scriptgroup] = []
        ajax = MakeAjaxButton(
            'a', MakeURL(EditScript) + u'?sid=%s&scriptname=%s' %
            (o.id, o.scriptname), u'编辑', 'open', Random)
...
def ScriptsManage(request):
    ...
    table = {'scripts': ['', 'scriptname', 'scriptgroup',
                         'scriptdescribe', 'creater', 'createtime']}
    alias = {u'脚本': [checkall, u'脚本名', u'目标组', u'脚本用途', u'修改者', u'修改时间']}
    thcss = ["width:100px", "width:100px", 'width:100px',
             '', 'width:100px', 'width:120px']  # 列的样式
    nosort = [True, 0]  # 不排序的列
    ...
    for o in scripts.objects.all():
        checkbox = u'''<input type='checkbox' class='checkbox' name='checkbox' value='%s__%s__%s__%s__%s'/>''' % (
            'scripts', 'scriptname', o.scriptname, 'id', o.id)
        Edit = MakeAjaxButton('a', EditScriptURL + u'?sid=%s&scriptname=%s' %
                              (o.id, o.scriptname), u'编辑', 'open', Random)
        Download = MakeAmark(u'下载', DownloadScriptURL +
                             '?sid=%s&scriptname=%s' % (o.id, o.scriptname))
        td = [checkbox + Download + ' ' + Edit, o.scriptname, o.scriptgroup,
              o.scriptdescribe, o.creater, DateTimeToStrTime(o.createtime), ]
...
def DelScript(request):
    ...
    if action == "Del":
        ...
        button = [
            [u'确定删除', MakeURL(DelScript) + u'?action=Del', 'load', u'删除', 'center', 3]]
...
def EditScript(request):
    ...
    if request.method == 'POST':
        ...
        if Sid and Scriptname and Describe and Scripttext:
            scripts.objects.filter(
                id=Sid).update(scriptname=Scriptname, scriptgroup=Scriptgroup,
                               scriptdescribe=Describe, creater=Creater, scripttext=Scripttext, createtime=Createtime)
    ...
    if Sid:
        ...
        button = [[u'确定修改', MakeURL(EditScript) + u'?sid=%s' % Sid, 'load'],
                  [u'复制', MakeURL(AddScript) + u'?sid=%s&scriptname=%s&action=copy' %
                   (Sid, Scriptname), 'submit', u'复制 %s ?' % Scriptname],
                  ]
...
def AddScript(request):
    ...
    Random = int(time.time() * 100)
    if request.method == 'POST':
        Action, Scriptname, Describe, Scriptgroup, Scripttext = GetPost(
            request,
        ...
        if Scriptname and Describe and Scripttext:
            ...
            o = scripts.objects.create(
                scriptname=Scriptname, scriptgroup=Scriptgroup,
                scriptdescribe=Describe, creater=Creater, scripttext=Scripttext, createtime=Createtime)
            if Action == "copy":
