...
class GSFileSystem():
    ...
    def __init__(self, request, chroot=FileManagerChroot):
        ...
        self.Absfilename = os.path.join(
            self.AbsDirPath, os.path.basename(filename))
    ...
    def unZipItem(self, request):
        ...
        # return HttpResponse('{result: \'0\' , gserror: \'can\\\'t uzzip %s
        # \'}'%Absfilename)
    ...
    def writefile(self, request):
        ...
        if filenContent:
            open(self.Absfilename, 'wb').write(
                UnquoteUTF8(filenContent).encode('utf-8'))
            return HttpResponse('{result: \'1\'}')
        ...
        try:
            ...
            if files:
                for filename in files:
                    ...
                    Abstarget = os.path.join(
                        self.AbsDirPath, os.path.basename(chrootfilename))
        ...
        if os.path.exists(Dir):
            ...
            for i in File:
                ...
                if os.path.isdir(AbsPath):
                    ...
                    html.append(
                        u'gsdirs.push(new gsItem("2", "%s", "%s", "%s", "%s", "dir", "%s"));' % tup)
                elif os.path.isfile(AbsPath):
                    ext = i.rfind('.')
                    ext = i[ext + 1:].lower() if ext >= 0 else 'unknown'
                    tup = (i, ChrootPath, os.path.getsize(AbsPath),
                           PathMd5, ext, PathMtime)
                    html.append(
                        u'gsfiles.push(new gsItem("1", "%s", "%s", "%s", "%s", "%s", "%s"));' % tup)
...
def FileManager(request):
    ...
    try:
        ...
        DownloadURL = 'http://%s:%s/%s' % (IP,
                                           request.META['SERVER_PORT'], FileManagerChroot)
    except:
        pass
