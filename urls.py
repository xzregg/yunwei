...
if not user.objects.all():  # 生成默认账户admin
    user.objects.create(
        createuser='admin', loginname='admin', username='admin',
        usergroup='', password=Sha1Md5('admin'), permission='0')


autourl = ['',
           (r'^favicon\.ico$', 'django.views.generic.simple.redirect_to',
            {'url': '/static/images/favicon.ico'}),
           (r'^$', 'admin.views.returnIndex',
            {'redirectUrl': html.MakeURL(index)}),
           (r'^login[/]?$', 'admin.views.login'),
           (r'^logout[/]?$', 'admin.views.logout'),
           (r'^get_check_code_image/$',
            'admin.check_code.get_check_code_image'),  # 验证码
           (r'^(static|media)/(?P<path>.*)$', 'django.views.static.serve',
            {'document_root': 'static'}),  # js和css的静态文件
           (r'^%s/(?P<path>.*)$' % UploadDir, 'libs.sendfile.download',
            {'document_root': '%s' % UploadDir}),  # 文件下载
           ]
...
def GetPyFileFunc(Dirlist):
    ...
    PyFileList = [os.path.join(dirpath, filename) for dir in Dirlist if os.path.isdir(dir)
                  for dirpath, dirs, files in os.walk(dir) for filename in files if filename.endswith('.py')]
    funcdict = {}
...
def MakeURL(viewsdict):
    ...
    for k, v in viewsdict.iteritems():
            ...
            if view:  # 找到的话把视图对应的url加上
                ...
                autourl.append(
                    ('%s' % view.viewurl, requires_login(v, view.viewname)))
            else:  # 数据库没有记录,插入记录
                Title = v.__doc__.split(
                    '\n')[1].lstrip().rstrip() if v.__doc__ else k
                displaytype = 1 if Title in DefaultShow else 0
                # 默认全部功能不显示
                x = k.split('.')
                ...
                o = views.objects.create(
                    title=Title, accessurl=accessurl, viewurl=viewurl, viewname=k,
                    permission=0, groupname=defaultgroup, loadcheck=1, displaytype=displaytype)
                o.permission = o.id
                o.save()
