...
def GetUrlPermission(url):
    ...
    for o in views.objects.filter(accessurl=url):
        url = o.accessurl
        if o.viewurl.find('^') == 0:
            UrlPermission['normalurl'][url] = {
                'permission': o.permission, 'logtype': o.logtype, 'title': o.title}
        else:
...
def requires_login(view, accessurl='', *args, **kwargs):  # 登录与权限检查
    def new_view(request, *args, **kwargs):
        ...
        if request.session.get('uid', '') and (Now - request.session.get('accesstime', Now)).seconds < _sessiontimeout:
            ...
            if U_P == -1:
                return HttpResponse(u'你已被锁定!<a href="/logout/">登出</a>')
...
def login(request):
    ...
    if request.method == 'POST' and request.session.get('verify', ''):
        ...
        if verify == checkcode and len(password) == 256:  # Rsa1024为加密后长度为256
            ...
            try:
                user_objects = user.objects.get(
                    loginname=loginname, password=Sha1Md5(password))
                ...
                msg = u'登录失败,你还有[%s]次机会!' % (
                    LoginFailNums - request.session['loginfail'])
                Log(request, u'登录失败', u'认证失败', loginname)
        else:
            request.session['loginfail'] += 1
            msg = u'验证错误,你还有[%s]次机会!' % (
                LoginFailNums - request.session['loginfail'])
...
def DelUser(request):
    ...
    button = [
        [u'锁定%s' % User, MakeURL(DelUser) + u'?action=lock&uid=%s&user=%s' %
         (Uid, User), 'load', u'锁定', 'center', 3],
        [u'解锁%s' % User, MakeURL(DelUser) + u'?action=unlock&uid=%s&user=%s' %
         (Uid, User), 'load', u'解锁', 'center', 3],
        [u'删除%s' % User, MakeURL(DelUser) + u'?action=delete&uid=%s&user=%s' %
         (Uid, User), 'load', u'删除', 'center', 3]
    ]
...
def UserManage(request):
    ...
    Random = int(time.time() * 100)
    alias = {u'用户管理':
             [u'管理', u'登录名', u'用户名', u'用户组', u'创建者', u'登录时间', u'登录IP']}
    table = {'user_t': ['manage', 'loginname', 'username',
                        'usergroup', 'createuser', 'logintime', 'loginip']}
    thcss = ['', '', '', 'width:150px']
    ...
    for o in user.objects.all():
        DateTime = ''
        if o.logintime:
            DateTime = datetime.datetime.strftime(o.logintime, TimeFormat)
        ajax = MakeAjaxButton(
            'a', MakeURL(ShowPermission) + u'?uid=%s&user=%s' %
            (o.id, o.loginname), u'查看权限', 'load', Random, '', 'center')
        ajax += MakeAjaxButton('a', MakeURL(EditUser) + u'?uid=%s&user=%s' %
                               (o.id, o.loginname), u'编辑', 'open', Random,)
        Lock = u'解锁' if o.lock == True else u'锁定'
        ajax += MakeAjaxButton('a', MakeURL(DelUser) + u'?uid=%s&user=%s' %
                               (o.id, o.loginname), Lock, 'load', Random)
        tds = [ajax, o.loginname, o.username, o.usergroup,
               o.createuser, DateTime, o.loginip]
        tdattrs = []
...
def GetPermission(IntPermission, showall=True):
    ...
    for v in views.objects.all():
        if not v.groupname in Views.keys():
            Views[v.groupname] = []
        # 拥有这个功能的权限
        checked = True if CheckBin(IntPermission, v.permission) else False
        if showall or checked:  # 这个用户拥有这个权限，显示
            Views[v.groupname].append(
                MakeCheckbox(v.id, v.title + '[%s]' % v.permission, checked))
    return Views
...
def ShowPermission(request):
    ...
    button = [
        [u'修改权限', MakeURL(ChangePermission) + u'?uid=%s&user=%s&gid=%s&group=%s' %
         (Uid, User, Gid, Group), 'load', '', '', 3]
    ]
...
def AddUserGroup(request):
    ...
    '''
    Random = int(time.time() * 100)
    usergroupname, viewslist = GetPost(
        request, ['usergroupname', 'checkbox'], [1])
    if viewslist and usergroupname:
        if not usergroup.objects.filter(groupname=usergroupname):
            permission = views.MakePermission(viewslist)
            usergroup.objects.create(
                groupname=usergroupname, permission=permission)
...
def DelUserGroup(request):
    ...
    button = [
        [u'删除[%s]' % Group, MakeURL(DelUserGroup) + u'?action=delete&gid=%s&group=%s' %
         (Gid, Group), 'load', u'删除', 'center', 3]]
    return render_to_response('Del.html', locals())
...
def UserGroupManage(request):
    ...
    for o in usergroup.objects.all():
        ...
        ajax1 = MakeAjaxButton(
            'a', MakeURL(ShowGroupUser) + u'?gid=%s&group=%s' %
            (o.id, o.groupname), u'查看组成员[%s]' % UserNums, 'load', Random)
        ajax2 = MakeAjaxButton(
            'a', MakeURL(ShowPermission) + u'?gid=%s&group=%s' %
            (o.id, o.groupname), u'查看组权限', 'load', Random, '', 'center')
        ajax3 = MakeAjaxButton(
            'a', MakeURL(DelUserGroup) + u'?gid=%s&group=%s' %
            (o.id, o.groupname), u'删除用户组', 'load', Random)
...
def ShowGroupUser(request):
    ...
    '''
    Random = int(time.time() * 100)
    Gid, Group, UserList, Action = GetPost(
        request, ['gid', 'group', 'checkbox', 'action'], [2])
    if Gid and Group:
        if Action == 'change':
        ...
        for o in user.objects.all():  # 列出组里用户
            ...
            UserCheckBox.append(
                MakeCheckbox(o.id, '%s[%s]' % (o.loginname, o.username), checked))
        UserCheckBox = ''.join(UserCheckBox)
        button = [[u'修改[%s]成员' % Group, MakeURL(
            ShowGroupUser) + u'?gid=%s&group=%s&action=change' % (Gid, Group), 'load', u'修改成员', '', 3]]
...
def ViewsLog(request):
    ...
    if action:
        ...
        logtype, Retmsg = (1, u'设置记录日志成功!') if action == "1" else (
            0, u'取消记录日志成功!')
        views.ViewsLog(Selects, logtype)
        return HttpResponse(Retmsg)
    else:
        button = [
            [u'记录日志', MakeURL(ViewsLog) + u'?action=1',
             'load', u'记录日志', '', 3],
            [u'取消记录', MakeURL(ViewsLog) +
             u'?action=0', 'load', u'取消记录', '', 3],
        ]
...
def ViewsShow(request):
    ...
    if action:
        Selects = [Vid] if Vid else [x.split('__')[-1] for x in Selects if x]
        displaytype, Retmsg = (
            1, u'设置菜单项成功!') if action == "1" else (0, u'取消菜单项成功!')
        views.ViewsShow(Selects, displaytype)
        return HttpResponse(Retmsg)
    else:
        button = [
            [u'设置菜单项', MakeURL(ViewsShow) + u'?action=1',
             'load', u'设置菜单项', '', 3],
            [u'取消菜单项', MakeURL(ViewsShow) +
             u'?action=0', 'load', u'取消菜单项', '', 3],
        ]
        return render_to_response('ViewsAction.html', locals())
...
def CheckViewPermission(request):
    ...
    if action:
        Selects = [Vid] if Vid else [x.split('__')[-1] for x in Selects if x]
        cp, Retmsg = (True, u'设置权限检测成功!') if action == "1" else (
            False, u'取消查权成功!')
        views.CheckViewPermission(Selects, cp)
        return HttpResponse(Retmsg)
    else:
        button = [
            [u'权限检测', MakeURL(CheckViewPermission) +
             u'?action=1', 'load', u'权限检测', '', 3],
            [u'取消查权', MakeURL(CheckViewPermission)
             + u'?action=0', 'load', u'取消查权', '', 3],
        ]
        return render_to_response('ViewsAction.html', locals())
...
def ViewsManage(request):  # 视图管理
    ...
    alias = {u'视图管理': [checkbox, u'功能名', u'访问URL',
                       u'DjangoURL', u'视图名', u'组名', u'查权', u'菜单', u'日志']}
    table = {'views': ['checkall', 'title', 'accessurl', 'viewurl',
                       'viewname', 'groupname', 'permission', 'displaytype', 'logtype']}  # 对应数据库
    T = Table(table, alias)  # 第3列为主键
    for o in views.objects.all():
        if o.viewname:
            ajax = u'<a href = "%s" target = "_blank">%s</a>' % (o.accessurl,
                                                                 u'访问')
        else:
            ajax = MakeAjaxButton(
                'a', MakeURL(DelPermission) + u'?pid=%s&title=%s' %
                (o.id, o.title), u'删除', 'load', Random)
        checkbox = MakeCheckbox('%s__%s__%s__%s' %
                                ('title', o.title, 'id', o.id), '')
        phtml, p = (u'取消', 0) if o.permission else (u'设置', 1)
        Permission = MakeAjaxButton(
            'a', MakeURL(CheckViewPermission) + u'?vid=%s&action=%s' %
            (o.id, p), phtml, 'load', Random, u'修改?', '', 2)
        mhtml, m = (u'取消', 0) if o.displaytype == 1 else (u'设置', 1)
        isMenu = MakeAjaxButton(
            'a', MakeURL(ViewsShow) + u'?vid=%s&action=%s' %
            (o.id, m), mhtml, 'load', Random, u'修改?', '', 2)
        lhtml, l = (u'取消', 0) if o.logtype else (u'设置', 1)
        isLog = MakeAjaxButton('a', MakeURL(ViewsLog) + u'?vid=%s&action=%s' %
                               (o.id, l), lhtml, 'load', Random, u'修改?', '', 2)
        tds = [checkbox + ajax, o.title, o.accessurl, o.viewurl,
               o.viewname, o.groupname, Permission, isMenu, isLog]
        tdattrs = ['', 'class="EditTD"', 'class="EditTD"', 'class="EditTD"',
                   '', 'class="EditTD"', 'class=""', 'class=""', 'class=""']
        T.row_add(o.id, tds, tdattrs)
    T.done()
    EditAjaxUrl = MakeURL(Modify)
    button = [[u'增加权限视图', MakeURL(AddPermission), 'open'],
              [u'清除错误视图', MakeURL(ClearErrView) +
               u'?action=clearerrviews', 'load'],
...
def DelPermission(request):
    ...
    button = [
        [u'删除[%s]' % Title, MakeURL(DelPermission) + u'?action=delete&pid=%s&title=%s' %
         (Pid, Title), 'load', u'删除', '', 3]]
    return render_to_response('Del.html', locals())
...
def AddPermission(request):
    ...
    Random = int(time.time() * 100)
    if request.method == "POST":
        title, viewsselect, kv, groupname = GetPost(
            request, ['title', 'viewsselect', 'kv', 'groupname'])
        if kv and title:
            views.AddPermission(title=title, accessurl=viewsselect, viewurl=kv,
                                viewname="", groupname=groupname, permission=0, displaytype=3, loadcheck=3)
...
def Log(request, title, info, who=''):
    ...
    ip = request.META['REMOTE_ADDR']
    logs.objects.create(title=title, who=who,
                        logdatetime=Now, info=info, ip=ip)
...
def ClearLogs(request):
    ...
    button = [
        [u'确定清除全部日志', MakeURL(ClearLogs) + u'?action=clear', 'load', u'清除全部日志', 'center', 3]]
...
def ShowLogs(request):
    ...
    查看日志
    '''
    Search, Page, Stime, Etime = GetPost(
        request, ['search', 'page', 'stime', 'etime'])
    ...
    # 对应数据库
    table = {'logs': ['id', 'who', 'logdatetime', 'ip', 'title', 'info']}
    thcss = ['width:20px', 'width:60px', 'width:160px', 'width:100px',
             'width:100px;text-align:center', 'text-align:left']
    T = Table(table, alias, thcss)
...
def index(request):
    ...
    logintime = DateTimeToStrTime(
        request.session.get('logintime', datetime.datetime.now()))
