...
class user(models.Model):
    ...
    def ChangePermission(cls, Uid, viewslist):  # 改变用户权限
        ...
        o.permission = 0 if o.username == 'admin' else views.MakePermission(
            viewslist)
        o.save()
...
class usergroup(models.Model):
    ...
    def ChangePermission(cls, Gid, viewslist):  # 改变用户组权限
        cls.objects.filter(id=Gid).update(
    ...
    def DelUserGroup(cls, Gid):  # 删除用户组
        ...
        for o in user.objects.filter(usergroup__contains=GroupName):
            o.usergroup = o.usergroup.replace(
                '%s,' % GroupName, '').replace(GroupName, '')
            o.save()
        go.delete()
        return GroupName
    ...
    def ChangeGroupUser(cls, Gid, UserList):  # 修改组用户
        ...
        for userobj in user.objects.all():
            userobj.usergroup = userobj.usergroup.replace(
                '%s,' % groupname, '')
...
class views(models.Model):
    ...
    groupname = models.CharField(
        max_length=50, blank=True, verbose_name="菜单组名")
...
class logs(models.Model):
    ...
    def SearchLog(cls, Page, ItemNum, Stime, Etime, text):
        ...
        o = cls.objects.filter(logdatetime__range=(std(Stime or '1970-01-01 00:00:00'), std(Etime))).filter(Q(info__icontains=text)
                                                                                                            | Q(title__icontains=text) | Q(ip__icontains=text) | Q(who__icontains=text)).order_by('-id')
        Tol = o.count()
...
# old------------------------------------------------------------
    ...
    def GetCount(text):
        cursor = connection.cursor()
        sql = '''select count(id) from admin_logs where title like '%%%%%s%%%%' or info like '%%%%%s%%%%' ''' % (
            text, text)
        cursor.execute(sql)
    ...
    def FilterOR(text, s, t, stime, etime):
        cursor = connection.cursor()
        sql = '''select * from admin_logs where (title like '%%%%%s%%%%' or info like '%%%%%s%%%%') and logdatetime >= '%s' and logdatetime <= '%s' order  by 'id' desc limit %s offset %s''' % (text,
                                                                                                                                                                                                 text, stime, etime, t, s,)
        cursor.execute(sql)
